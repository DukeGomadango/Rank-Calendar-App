...
import type { CalendarWithModalProps, DayData } from "./types";
import { runWeekGridKeyboardAction } from "./weekGridKeyboardHandler";

dayjs.locale("ja");

export function useCalendarWithModalLogic({
  calendarName,
  monthLabel,
  currentMonthParam,
  currentWeekStart,
  ...
  const handleWeekGridKeyDown = useCallback(
    (e: ReactKeyboardEvent<HTMLElement>) => {
      void runWeekGridKeyboardAction(e, {
        view,
  ...
  const onMonthDayActivate = useCallback(
    (day: DayData) => {
